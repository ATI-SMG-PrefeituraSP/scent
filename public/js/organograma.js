  ...
  function orgChart() {
    ...
    var lineColor = "#9AD1D4", // Color of the connection lines (global for all lines)
      boxWidth = 120, // Box width (global for all boxes)
      boxHeight = 50, // Box height (global for all boxes)
      hSpace = 50, // Horizontal space in between the boxes (global for all boxes)
      vSpace = 40, // Vertical space in between the boxes (global for all boxes)
      hShift = 15, // The number of pixels vertical siblings are shifted (global for all boxes)
      boxLineColor = "#9AD1D4", // Default box line color
      ...
      curshadowOffsetX = 0,
      curshadowOffsetY = 0,
      shadowColor = "#A1A1A1", //cor da sombra
      curtopradius = 0,
      curbotradius = 0,
      nodes = [],
      theCanvas,
    ...
    orgChart.prototype.addNode = function (id, parent, ctype, text, bold, url, linecolor, fillcolor, textcolor, img,
      imgalign) {
      var imgvalign;
      ...
      if (y > 0) {
        for (n = nodes[p].vpos + boxHeight / 2 + boxHeight + vSpace; n <= y; n += boxHeight + vSpace) {
          ...
          do {
            ...
            if (s >= 0) {
              debugOut("Overlap between a downline of '" + nodes[p].txt + "' at point (" + (nodes[p].hpos + boxWidth /
                  2) + ", " + n + ") and node '" + nodes[s].txt + "' (" + nodes[s].hpos + ", " + nodes[s].vpos +
                ")");
          ...
          if (j >= 0) {
            ...
            if (ui !== uj) {
              ...
              if (nodeUnderParent(i, ui) && nodeUnderParent(j, ui)) {
                ...
                while (nodes[ui].parent !== '' && nodes[ui].contype === 'u' && nodes[nodes[ui].parentix].usib.length ===
                  1) {
    ...
    positionTree = function (p) {
      ...
      for (v = 0; v < nodes[p].lsib.length; v++) {
        ...
        do {
          ...
          if (o < 0) {
            o = getNodeAtUnequal(nodes[s].hpos + boxWidth + minDistBetweenLineAndBox, nodes[s].vpos +
              minDistBetweenLineAndBox, s);
          }
          if (o < 0) {
          ...
          if (o >= 0) {
            debugOut("New lsib '" + nodes[s].txt + "' (" + nodes[s].hpos + ", " + nodes[s].vpos +
              ") has overlap with existing node '" + nodes[o].txt + "' (" + nodes[o].hpos + ", " + nodes[o].vpos +
              ")");
      ...
      for (v = 0; v < nodes[p].rsib.length; v++) {
        ...
        do {
          ...
          if (o >= 0) {
            debugOut("New rsib '" + nodes[s].txt + "' (" + nodes[s].hpos + ", " + nodes[s].vpos +
              ") has overlap with existing node '" + nodes[o].txt + "' (" + nodes[o].hpos + ", " + nodes[o].vpos +
              ")");
            h = nodes[s].hpos - nodes[o].hpos;
            h = Math.abs(h);
      ...
      if (n > 0) {
        ...
        for (i = 0; i < nodes[p].rsib.length; i++) {
          ...
          if (w) {
            debugOut("Right tree '" + nodes[nodes[p].rsib[i]].txt +
        ...
        for (h = 0; h < nodes[p].usib.length; h++) {
          ...
          n = 0;
          do {
            o = getNodeAtUnequal(nodes[s].hpos - minDistBetweenLineAndBox, nodes[s].vpos + minDistBetweenLineAndBox,
              s);
            if (o < 0) {
              o = getNodeAtUnequal(nodes[s].hpos + boxWidth + minDistBetweenLineAndBox, nodes[s].vpos +
                minDistBetweenLineAndBox, s);
            ...
            if (o >= 0) {
              debugOut("New usib '" + nodes[s].txt + " at (" + nodes[s].hpos + ", " + nodes[s].vpos +
                ")' has overlap with existing node '" + nodes[o].txt + "' at (" + nodes[o].hpos + ", " + nodes[o].vpos +
                ")");
              w = nodes[o].hpos - nodes[s].hpos + boxWidth + hSpace;
    ...
    dumpNodes = function () {
      var i;
      for (i = 0; i < nodes.length; i++) {
        console.log(i + ": " + nodes[i].parentix + " at(" + nodes[i].hpos + "," + nodes[i].vpos + ") usib = " + nodes[
            i].usib.length + " lsib = " + nodes[i].lsib.length + " rsib = " + nodes[i].rsib.length + "  " + nodes[i]
          .txt + ", parent = " + nodes[i].parent + ", parentix = " + nodes[i].parentix);
      }
    };
